...
def rank_crawler():
    ...
    for x in range(13):
        for j in range(3):
            ...
            class Video:
                def __init__(self, aid, author, coins, play, pts, title, video_review, pic, reprint):
                    self.aid = aid
                    self.title = title
                    self.author = author
                    self.coins = coins
                ...
                def to_csv(self):
                    return [self.aid, self.author, self.coins, self.play, self.pts, self.title, self.video_review, self.pic, self.reprint]

                @staticmethod
                def csv_title(self):
                    return ['视频id', 'up主', '投币数', '播放数', '综合得分', '视频标题', '视频弹幕数', '视频封面', '原创']
            ...
            for i in range(len(rank_list)):
                ...
                if no_reprint == 1:  # 是否原创
                    ...
                    reprint = "转载"
                v = Video(aid, author, coins, play, pts, title, video_review, pic, reprint)
                videos.append(v)

            file_name = '{}{}top100.csv'.format(rid1, day1)  # 保存视频信息的文件
